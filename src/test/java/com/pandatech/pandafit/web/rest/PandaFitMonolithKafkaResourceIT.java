...
import com.pandatech.pandafit.config.KafkaProperties;
...
class PandaFitMonolithKafkaResourceIT {
    ...
    void setup() {
        ...
        PandaFitMonolithKafkaResource kafkaResource = new PandaFitMonolithKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/panda-fit-monolith-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/panda-fit-monolith-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
